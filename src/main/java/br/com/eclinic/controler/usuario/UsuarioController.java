...
public class UsuarioController extends EclinicController {
	...
	public String exibirAlterar(@PathVariable Long pk, Model model, HttpServletRequest request) throws Exception {
		...
		if (usuarioAlteracao != null) {
			...
			usuarioAlteracao
					.setCodigoTipoUsuarioTransiente(String.valueOf(usuarioAlteracao.getTipoUsuario().getCodigo()));
	...
	public ModelAndView alterar(@ModelAttribute Usuario usuario, BindingResult result, Model model,
	 ...
	 * HttpServletRequest request) throws NumberFormatException, NegocioException {
	 * 
	 * UsuarioValidator validator = new UsuarioValidator();
	 * validator.validate(usuario, result);
	 * 
	 ...
	 * ModelAndView("usuario_alterar"); mav.addObject(MODEL_ATTR_ENTIDADE, usuario);
	 * 
	 * return mav; } else { Usuario usuarioBanco =
	 * service.consultarPorLogin(usuario.getLogin()); if (usuarioBanco != null &&
	 * usuarioBanco.getId() != usuario.getId()) { ModelAndView mav = new
	 * ModelAndView("usuario_alterar");
	 * 
	 * model.addAttribute("perfis",
	 * perfilService.listarPorCliente(usuarioLogado.getCliente()));
	 * 
	 * mav.addObject(MODEL_ATTR_ENTIDADE, usuario); mav.addObject(MESSAGE_ERROR,
	 * getMensagem("usuario.existente")); mav.addObject("perfis",
	 * perfilService.listarPorCliente(usuarioLogado.getCliente())); return mav; }
	 * else { usuario.setCliente(usuarioLogado.getCliente());
	 * service.alterar(usuario); redirectAttributes.addFlashAttribute(MESSAGE,
	 * getMensagem("alterar.sucesso")); ModelAndView retorno = new
	 * ModelAndView("redirect:/usuarios"); return retorno; } } }
	 */

	/*
	 * @RequestMapping(value = URL_REQUEST_GERENCIAR_CLIENTE + "/{pk}", method =
	 * RequestMethod.GET) public String exibirGerenciarCliente(@PathVariable Long
	 * pk, Model model, HttpServletRequest request) throws Exception { Usuario
	 * usuarioAlteracao = service.buscar(pk); if (usuarioAlteracao != null) {
	 * model.addAttribute(MODEL_ATTR_ENTIDADE, usuarioAlteracao);
	 * 
	 * model.addAttribute("clientes", clienteService.listar()); return
	 * "usuario_gerenciar_cliente"; } else { return "redirect:../usuarios"; } }
	 */

	/*
	 * @RequestMapping(value = URL_REQUEST_GERENCIAR_CLIENTE, method =
	 * RequestMethod.POST) public String gerenciarCliente(@ModelAttribute Usuario
	 * usuario, Model model) throws Exception {
	 * 
	 ...
	 * "Erro: Selecione um cliente."); model.addAttribute(MODEL_ATTR_ENTIDADE,
	 * usuarioAlteracao);
	 * 
	 * }
	 * 
	...
	public String excluirCliente(@ModelAttribute Usuario usuario, Model model) throws Exception {

		Usuario usuarioAlteracao = service.buscar(usuario.getId());

		for (int x = 0; x < usuarioAlteracao.getClientesPermissao().size(); x++) {
			if (usuarioAlteracao.getClientesPermissao().get(x).getId() == usuario.getClienteAdicionar().getId()) {
	...
	private boolean isClienteValido(List<Cliente> clientesPermissao, Cliente clienteAdicionar) {
	...
	public String excluir(@ModelAttribute Usuario usuario, Model model, final RedirectAttributes redirectAttributes) {
	...
	public String resetarSenha(@ModelAttribute Usuario usuario, Model model,
		...
		try {
			...
			redirectAttributes.addFlashAttribute(MESSAGE, getMensagem("resetar.senha.sucesso"));
		} catch (NegocioException e) {
			e.printStackTrace();
			redirectAttributes.addFlashAttribute(MESSAGE_ERROR, e.getMessage());
		} catch (DataIntegrityViolationException e) {
	...
	/*
	 * @RequestMapping(value = URL_REQUEST_SALVAR_PERFIL, method =
	 * RequestMethod.POST) public ModelAndView salvarPerfilUsuario(Model
	 * model, @ModelAttribute Perfil perfil, BindingResult result, Usuario usuario,
	 * final RedirectAttributes redirectAttributes, HttpServletRequest request)
	 * throws Exception {
	 * 
	 * PerfilValidator validator = new PerfilValidator(); validator.validate(perfil,
	 * result);
	 * 
	 * Usuario usuarioLogado = HomeController.getUsuarioLogado(request);
	 * 
	 * if (result.hasErrors()) { ModelAndView mav = new
	 * ModelAndView("usuario_incluir"); mav.addObject("perfil", perfil);
	 * mav.addObject("usuario", usuario); return mav; } else { List<Perfil>
	 * usuariosBanco = perfilService.consultarPorDescricao(perfil.getDescricao(),
	 * usuarioLogado.getCliente()); if(usuariosBanco.size() != 0){ ModelAndView mav
	 * = new ModelAndView("usuario_incluir"); mav.addObject("perfil", perfil);
	 * mav.addObject(MESSAGE_ERROR, getMensagem("perfil.existente"));
	 * mav.addObject("usuario", usuario); return mav; }else{
	 * perfil.setCliente(usuarioLogado.getCliente()); perfilService.salvar(perfil);
	 * redirectAttributes.addFlashAttribute(MESSAGE,
	 * getMensagem("inclusao.sucesso")); ModelAndView retorno = new
	 * ModelAndView("redirect:/usuario_incluir"); retorno.addObject("usuario",
	 * usuario); retorno.addObject("perfil", perfil); return retorno; } }
	...
	public ModelAndView salvarPerfilUsuario(Model model, @ModelAttribute UsuarioDTO usuario, BindingResult result,
			final RedirectAttributes redirectAttributes, HttpServletRequest request) throws Exception {// @ModelAttribute
																										// Usuario
																										// usuario,
...
//		PerfilValidator validator = new PerfilValidator();
		...
		if (result.hasErrors()) {
			...
			List<Perfil> usuariosBanco = perfilService.consultarPorDescricao(perfil.getDescricao(),
					usuarioLogado.getCliente());
