					...
					<div class="page-title">
						<h4>
							<i class="icon-arrow-left52 position-left"></i> <span
								class="text-semibold">Aprovações</span> - Consultas
						</h4>
					</div>
				</div>
			...
			<div class="content">
				...
				<c:if test="${empty solicitacoes}">
					<div class="alert alert-info-diogo">
						...
						ATENÇÃO! Essa consulta não existe. Verifique o código digitado ou cadastre uma nova consulta.
					</div>
				</c:if>
				...
				<div class="panel panel-flat">
					<div class="panel-body">
						<form:form method="POST" class="form-horizontal" role="form"
							action="${raiz}consulta" modelAttribute="solicitacaoConsulta">

							<div class="row">
								<div class="col-xs-1">
									<label for="field_1">Data da Solicitação</label>
								</div>
								<div class="col-xs-2">
						...
						<tbody>
							<c:forEach var="sol" items="${solicitacoes}">
								<tr>
									...
									<c:if test="${sol.status eq 'CONSULTA AGENDADA'}">
										<c:url value="bg-success" var="classe" />
									</c:if>

									<td><c:out value="${sol.dataFormatada}" /></td>
									<td><c:out value="${sol.codigo}" /></td>
									<td><span class="label ${classe}"><c:out value="${sol.status}" /></span></td>
									<td><a href="${raiz}consulta/visualizar/${sol.id}"
